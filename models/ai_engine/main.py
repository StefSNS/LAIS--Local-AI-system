...
from llm_engine import chat, get_available_models, switch_model, get_current_model
from plugin_manager import load_all, start_watcher, plugins
from knowledge.memory import opencode_sessions
...
class DesktopGUI(ctk.CTk):
    ...
    def _discover_plugins(self):
        """Auto-discover plugins from plugins/ dir and tool_registry.json."""
        self.available_plugins = [("Search", "search:", "#00d4ff")]
        prefix_colors = ["#89d185", "#f9826c", "#d2a8ff", "#79c0ff", "#ffd700", "#ff6b6b", "#6bcbff", "#c084fc"]
        color_idx = 1
        ...
        for pname in plugin_files:
            prefix = pname.replace("_", " ") + ":"
            if not any(prefix == ap[1] for ap in self.available_plugins):
                color = prefix_colors[color_idx % len(prefix_colors)]
                self.available_plugins.append((pname.replace("_", " ").title(), f"{pname}:", color))
                color_idx += 1
    ...
    def _build_sidebar(self):
        ...
        self.model_label.grid(row=3, column=0, padx=15, pady=(5, 2), sticky="w")
        ...
        try:
            ...
            self.model_dropdown.grid(row=4, column=0, padx=15, pady=(0, 10), sticky="ew")
        ...
        self.plugin_label.grid(row=5, column=0, padx=15, pady=(5, 5), sticky="w")
        ...
        self.plugin_dropdown.grid(row=6, column=0, padx=15, pady=(0, 10), sticky="ew")
        self.plugin_dropdown.set("Select Plugin...")
        
        self.status_frame = ctk.CTkFrame(self.sidebar, fg_color="#0d0d0d", corner_radius=8)
        self.status_frame.grid(row=7, column=0, padx=15, pady=15, sticky="ew")
        ...
        self.cpu_label.pack(pady=3)
        
        self.model_label_stat = ctk.CTkLabel(
            self.status_frame,
            text="",
            font=ctk.CTkFont(size=11),
            text_color="#8b8b8b"
        )
        self.model_label_stat.pack(pady=3)
        
        self.plugins_label_stat = ctk.CTkLabel(
            self.status_frame,
            text="",
            font=ctk.CTkFont(size=11),
            text_color="#8b8b8b"
        )
        self.plugins_label_stat.pack(pady=3)

        self.metrics_frame = ctk.CTkFrame(self.status_frame, fg_color="transparent")
        self.metrics_frame.pack(pady=(5, 0))
        ...
        self.msg_count_label.pack(side="left", padx=(0, 8))

        self.err_count_label = ctk.CTkLabel(
            self.metrics_frame,
            text="Err: 0",
            font=ctk.CTkFont(size=10),
            text_color="#6b6b6b"
        )
        self.err_count_label.pack(side="left", padx=(0, 8))
    ...
    def _on_model_selected(self, choice):
        """Handle model switcher dropdown selection."""
        if not choice or choice == get_current_model():
            return
        success = switch_model(choice)
        if success:
            self._add_message("system", f"Switched to model: {choice}")
        else:
            self._add_message("error", f"Failed to switch to model: {choice}")

    def update_unified_stats(self):
        """Update unified layer stats in the sidebar."""
        try:
            active_models = len(get_available_models())
            current = get_current_model()
            self.model_label_stat.configure(text=f"Model: {current} ({active_models} available)")
            
            loaded = len([k for k in plugins.keys()])
            self.plugins_label_stat.configure(text=f"Plugins: {loaded} loaded")
            
            self.msg_count_label.configure(text=f"Msgs: {self._msg_count}")
            self.err_count_label.configure(text=f"Err: {self._error_count}")
            self.token_count_label.configure(text=f"Tokens: {self._total_tokens}")
        except Exception:
            pass
        self.after(5000, self.update_unified_stats)
    ...
    def send_message(self):
        ...
        self._msg_count += 1
        self._total_tokens += len(q) // 4
        threading.Thread(target=self._safe_process_message, args=(q,), daemon=True).start()
    ...
    def _safe_process_message(self, q):
        """Error-boundary wrapper for message processing."""
        try:
            self._process_message(q)
        except Exception as e:
            self._error_count += 1
            err_msg = f"Error: {str(e)}"
            self.after(0, self._hide_typing)
            self.after(0, lambda: self._add_message("error", err_msg))
            try:
                opencode_sessions.save_session(
                    summary=f"Error processing: {q[:80]}",
                    phase="error",
                    completed=[],
                    pending=[str(e)]
                )
