...
def reminder(parameters: dict) -> str:
  ...
  <Principals>
    <Principal>
      ...
      <RunLevel>LeastPrivilege</RunLevel>
        ...
        result = subprocess.run(f'schtasks /Create /TN "{task_name}" /XML "{xml_path}" /F', shell=True, capture_output=True, text=True)
        try:
            os.remove(xml_path)
        except Exception:
            pass
        ...
        if result.returncode != 0:
            ...
            try:
                ...
                pass
            return "Could not schedule the reminder due to a system error."

        return f"Reminder set for {target_dt.strftime('%B %d at %I:%M %p')}."

    except ValueError:
        return "Could not understand that date or time format."
    except Exception as e:
        return f"Error scheduling reminder: {str(e)[:80]}"
