...
LAIS_PATH = Path(__file__).resolve().parent
...
import sqlite3
JARVIS_DB = os.environ.get("JARVIS_MEMORY_DB", str(Path(__file__).resolve().parent.parent / "Mark-XXXIX" / "memory" / "jarvis_memory.db"))
...
conn = sqlite3.connect(str(Path(__file__).resolve().parent / "knowledge" / "memory" / "unified_memory.db"))
cur = conn.cursor()
cur.execute("SELECT COUNT(*) FROM memory_entries")
